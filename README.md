...
# Fork 说明
原仓库已经两年左右没有被继续维护，由于本人工作摸鱼需要，将原仓库 fork 下来继续开发。
目前正在做面向摸鱼的主题分支(theme_management)。由于对前端和vscode插件开发都不熟悉，可能会有不少诡异的提交，请不要向我提问，因为我多半也不懂。由于就连这个插件的继续开发也是摸鱼性质，所以不保证提交的稳定性。

另外，原仓库因为缺少一项配置项，无法直接构建vsce，就顺便随意补充了影响构建的一个配置项。现在 theme_management 分支已经可以正常构建项目。
如果有更好的解决方式请告诉我。
...
2. 进入项目目录并Shift+右键打开 windows 终端或 powershell 等终端
...
5. build 插件
   准备工作：
   - 如果使用主分支
     添加以下代码到 `package.json` 文件中，`"keywords": [`的上一行
     ```json
      "activationEvents": [
      "*"
      ],
     ```
   - 如果使用 theme_management 分支，直接继续
  
   终端执行`npx vsce package`
   中途会遇到警告
